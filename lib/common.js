...
var dappAddress = "n1vUAveHtRgW4DWm7jyLKQ8W9VvRKDtMuJG";
 ...
 function cbAdd(resp) {        
    ...
    setInterval(function(){
      neb.api.getTransactionReceipt({hash: hash_value}).then(function(receipt) {
        ...
        if (result_trans == 1) {
          $('.result_trans').html('result: <p style="color: green"> sucess </p>');    
          ...
          $('.result_trans').html('result: <p style="color: blue"> pending </p>');                        
        } else {
          $('.result_trans').html('result: <p style="color: red"> fail </p>');    
          setTimeout(function(){ $('#transaction button').trigger('click') } , 2000);          
        }
    })}, 1000);    
}
